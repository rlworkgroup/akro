...
from akro.dict import Dict as AkroDict
from akro.theano.box import Box
from akro.theano.discrete import Discrete
from akro.theano.tuple import Tuple


class Dict(AkroDict):
    """Theano extension of akro.Dict."""

    def __init__(self, spaces):
        """
        Initialize akro.Dict.
        ...
        # Convert each space to a akro.space
        for key, space in self.spaces.items():
            self.spaces[key] = self._to_akro_space(space)
    ...
    def _to_akro_space(self, space):
        """
        Convert a gym.space to a akro.tf.space.

        Returns:
            space (akro.tf.spaces)
